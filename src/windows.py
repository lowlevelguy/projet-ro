...
class MainWindow(QWidget):
    ...
    def initUI(self):
        ...
        btn = [
            ActionButton(
                self.open_affect_window, "Affecter des étudiants à des stages", self
            ),
            ActionButton(
    ...
    def open_antenne_gui(self):
        script_path = getcwd() + "/antenne_gui.py"
        try:
...
class AffectWindow(QWidget):
    ...
    def setup(self):
        ...
        i_upload_btn.setToolTip(
            "Format:<br>- première colonne: titres des stages (nom de la colonne: Internships)<br>- deuxième colonne: capacités des stages (nom de la colonne: Capacities)<br>- le reste des colonnes spécifient les compétences requises"
        )
        s_upload_btn.setToolTip(
            "Format:<br>- première colonne: noms et prénoms des étudiants (nom de la colonne: Students)<br>- le reste des colonnes spécifient les compétences"
        )
    ...
    def affect(self):
        ...
        summary = pd.DataFrame(
            {"Students": s_names, "Affected Internships": i_titles[aff_indices]}
        )
...
class TaskAssignmentWindow(QWidget):
    ...
    def setup(self):
        ...
        m_upload_btn.setToolTip(
            "Format:<br>- première colonne: noms des machines (nom de la colonne: Machines)<br>- deuxième colonne: capacités des machines (nom de la colonne: Capacities)<br>- le reste des colonnes spécifient les capacités/caractéristiques des machines"
        )
        t_upload_btn.setToolTip(
            "Format:<br>- première colonne: noms et identifiants des tâches (nom de la colonne: Tasks)<br>- le reste des colonnes spécifient les exigences/ressources nécessaires pour chaque tâche"
        )
    ...
    def assign_tasks(self):
        try:
            ...
            summary = pd.DataFrame(
                {"Tasks": t_names, "Assigned Machines": m_titles[asg_indices]}
            )
